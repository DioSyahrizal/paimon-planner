...
const DECIMAL_STAT_KEYS = new Set<keyof StatBlock>(['er', 'cr', 'cd', 'elementalDmgBonus', 'healingBonus'])

function checkArtifactSetMatch(character: Character, build: RecommendedBuild): boolean {
  const equippedSetIds = character.artifacts.map((a) => a.setId)

  return build.artifactSets.some((option) =>
    option.sets.every((s) => {
      const count = equippedSetIds.filter((id) => id === s.setId).length
      return count >= s.pieces
    })
  )
}

function checkMainStatMatch(character: Character, build: RecommendedBuild): Record<ArtifactSlot, boolean> {
  const slots: ArtifactSlot[] = ['flower', 'feather', 'sands', 'goblet', 'circlet']
...
function checkWeaponTier(character: Character, build: RecommendedBuild): 'S' | 'A' | 'B' | 'unranked' {
...
function computeSubstatGaps(character: Character, build: RecommendedBuild): StatGap[] {
  const gaps: StatGap[] = []
  ...
  for (const stat of build.substatPriority) {
    ...
    const current = statKey && DECIMAL_STAT_KEYS.has(statKey) ? rawValue * 100 : rawValue
...
function computeSubstatCoverage(character: Character, build: RecommendedBuild): number {
  const top4 = build.substatPriority.slice(0, 4)
  const allSubstats = character.artifacts.flatMap((a) => a.subStats.map((s) => s.stat))
...
function computeOverallScore(
  ...
  const weaponScores: Record<string, number> = { S: 20, A: 14, B: 8, unranked: 0 }
  const weaponScore = weaponScores[weaponTier] ?? 0

  return Math.round(setScore + mainStatScore + substatScore + weaponScore)
}

export function analyzeGap(character: Character, build: RecommendedBuild): GapAnalysis {
  ...
  const overallScore = computeOverallScore(artifactSetMatch, mainStatMatch, substatCoverage, weaponTier)
